...
export const CageDailyHistoryModal = ({ cage }: CageDailyHistoryModalProps) => {
  ...
  const formatActivities = (activities: any[], type: string) => {
    ...
    const getIcon = () => {
      switch (type) {
        case 'alimentation': return <Fish className="h-4 w-4" />;
        case 'qualite_eau': return <Droplets className="h-4 w-4" />;
        case 'sante': return <Heart className="h-4 w-4" />;
    ...
    const getTitle = () => {
      switch (type) {
        ...
        case 'qualite_eau': return 'Qualité de l\'eau';
            ...
            <div key={index} className="p-2 bg-muted/50 rounded-md text-xs">
              ...
              {type === 'qualite_eau' && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    {activity.turbidite && (
                      <p><strong>Turbidité:</strong> {activity.turbidite} NTU</p>
                    )}
                  </div>
                  {activity.observations && (
                    <p className="mt-1"><strong>Observations:</strong> {activity.observations}</p>
                  )}
                </div>
              )}
              ...
              {type === 'sante' && (
                <div>
                  ...
                  <p><strong>Observations:</strong> {activity.observations}</p>
                  {activity.cause_presumee && (
                    <p><strong>Cause présumée:</strong> {activity.cause_presumee}</p>
                  )}
                  {activity.traitements && activity.traitements.length > 0 && (
                    <p><strong>Traitements:</strong> {activity.traitements.join(', ')}</p>
                  )}
                </div>
              )}
              ...
              {type === 'finance' && (
                <div>
                  ...
                  {activity.reference_document && (
                    <p><strong>Référence:</strong> {activity.reference_document}</p>
                  )}
                </div>
              )}
            </div>
  ...
  const hasActivities = (day: any) => {
    return (
      (day.alimentation && day.alimentation.length > 0) ||
      (day.qualite_eau && day.qualite_eau.length > 0) ||
      (day.sante && day.sante.length > 0) ||
      (day.ventes && day.ventes.length > 0) ||
      (day.finance && day.finance.length > 0)
    );
  };
                ...
                <div key={index} className="border rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold">
                      {format(new Date(day.date_activite), 'EEEE d MMMM yyyy', { locale: fr })}
                    </h3>
                  </div>
                  ...
                  {hasActivities(day) ? (
                    <div className="grid gap-4 md:grid-cols-2">
                      {day.alimentation && day.alimentation.length > 0 && 
                        formatActivities(day.alimentation, 'alimentation')}
                      {day.qualite_eau && day.qualite_eau.length > 0 && 
                        formatActivities(day.qualite_eau, 'qualite_eau')}
                      {day.sante && day.sante.length > 0 && 
                        formatActivities(day.sante, 'sante')}
                      {day.ventes && day.ventes.length > 0 && 
                        formatActivities(day.ventes, 'ventes')}
                      {day.finance && day.finance.length > 0 && 
                        formatActivities(day.finance, 'finance')}
                    </div>
                  ) : (
