...
const Predictions = () => {
  const { predictions, loading, generatePredictions, getPredictionsByCage } = useProductionPredictions();
  ...
  const filteredPredictions = selectedCage === 'all' 
    ? predictions 
    : getPredictionsByCage(selectedCage);
  ...
  const getPredictionStatus = (days: number) => {
    if (days <= 7) return { label: 'Récolte imminente', color: 'destructive' };
    if (days <= 30) return { label: 'Bientôt prêt', color: 'default' };
    if (days <= 60) return { label: 'En cours', color: 'secondary' };
    return { label: 'Début de cycle', color: 'outline' };
  };
    ...
    <div className="min-h-screen p-6 animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-2 mt-2 text-white/60">
          ...
          <span className="text-sm">Prédictions basées sur l'IA</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
        {harvestPredictions.map((prediction) => {
          ...
          return (
            <Card key={prediction.id} className="bg-card/80 backdrop-blur-md border-border/20 shadow-lg hover:shadow-xl transition-all duration-200">
              <CardHeader>
