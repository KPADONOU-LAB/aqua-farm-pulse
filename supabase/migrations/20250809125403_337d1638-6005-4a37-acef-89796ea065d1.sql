-- Triggers setup for full farm management reliability
-- Idempotent creation using DO blocks to avoid duplicate triggers
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_farm_settings_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_farm_settings_create_admin'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_farm_settings_create_default_dashboard'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_cages_log_changes'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_weighings_auto_adjust_feeding'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_health_remaining_fish'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_sales_remaining_fish'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_metrics_after_feeding'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_metrics_after_health'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_metrics_after_sales'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_metrics_after_costs'
  ) THEN
...
-- 8) Generic BEFORE UPDATE updated_at triggers for tables with updated_at column
DO $$
DECLARE
  rec RECORD;
BEGIN
    ...
    EXECUTE format('
      ...
      BEGIN
        IF NOT EXISTS (
          ...
          JOIN pg_class c ON c.oid = t.tgrelid
          JOIN pg_namespace n ON n.oid = c.relnamespace
          WHERE t.tgname = %L AND n.nspname = %L AND c.relname = %L
        ) THEN
          CREATE TRIGGER %I
        ...
        END IF;
      END$$;',
      'trg_' || rec.table_name || '_updated_at', rec.table_schema, rec.table_name,
      'trg_' || rec.table_name || '_updated_at', rec.table_schema, rec.table_name
    );
  END LOOP;
END$$;
